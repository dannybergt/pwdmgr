...
| Brute force | Per client+account 10/min, per client 30/min, per account 20 failures/15 min across clients, global Argon2 concurrency gate (503), per-user write token bucket (burst 60, refill 60/min — a steady 1 write/s never hits it, 80 in parallel do) | `LoginThrottle`, `Program.cs` |
