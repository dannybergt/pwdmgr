...
- **Phase:** **MVP vertical complete on feature branches** (2026-09-15): slices #3–#9 implemented, reviewed and verified as a stack of PRs (`feature/local-login-sessions` → `feature/keyring-vault-api` → `feature/secret-api` → `feature/web-unlock-flow`, plus `feature/keyring-crypto`). Login → passphrase enrol/unlock → create/read secrets works end-to-end in Chromium over TLS (Playwright golden path in repo and CI). Wave 1 (#1, #2) merged as PRs #24/#25. Plan: [`docs/developer/mvp-slice-plan.md`](docs/developer/mvp-slice-plan.md).
...
| network `pwdmgr-test`, container `pwdmgr-test-postgres` (no host port) | backend tests | throw-away; **running** during the 2026-09-15 MVP session, remove after the merge (`docker rm -f pwdmgr-test-postgres && docker network rm pwdmgr-test`), recreate per TESTING.md. |
...
| Docker Hub `dbergt/pwdmgr-web` | Dockerfile + CI job `docker-web` on `feature/web-unlock-flow`; first push happens with the merge to `main`. |
...
- [x] MVP wave 2–4 (#3 keyring crypto ADR-0009, #4 login + sessions ADR-0008, #5 keyring/vault API, #6 secret API, #7/#8 web client, #9 web image + TLS + CI) done on the PR stack; merge awaiting `FREIGABE`.
