...
- `tests/backend/Pwdmgr.Api.Tests` (xUnit v3 + `WebApplicationFactory`, 18 tests): login cookie
  ...
  **session expiry after a real 3-second TTL**, cross-origin POST → 403, storage holds only
  ...
  holder, cross-tenant invisibility (18 tests). Runs in CI. Verification catalogue:
