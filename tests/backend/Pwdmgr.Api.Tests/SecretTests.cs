...
public sealed class SecretTests(ApiFactory factory) : IClassFixture<ApiFactory>
{
    ...
    private static CreateSecretRequest NewSecret(byte fill = 0x10) => new("password", B64(40, fill), B64(120, fill), B64(60, fill), B64(32, fill));
    ...
    private async Task<(HttpClient Client, Guid VaultId)> NewUserWithVaultAsync()
    {
        ...
        var created = await client.PostAsJsonAsync(Vaults, new CreateVaultRequest("personal", B64(40), B64(92)), TestContext.Current.CancellationToken);
    ...
    public async Task Create_list_latest_new_version_delete_chain()
    {
        ...
        var v2Response = await client.PostAsJsonAsync(new Uri($"/api/v1/secrets/{v1.SecretId}/versions", UriKind.Relative), new NewVersionRequest(B64(130, 0x20), B64(60, 0x20), B64(32, 0x20), null), TestContext.Current.CancellationToken);
        ...
        Assert.Equal(B64(130, 0x20), (await client.GetFromJsonAsync<SecretVersionDto>(latestUri, TestContext.Current.CancellationToken))!.PayloadCiphertext);
    ...
    public async Task Foreign_vault_and_foreign_secret_are_404()
    {
        ...
        Assert.Equal(HttpStatusCode.NotFound, (await stranger.PostAsJsonAsync(new Uri($"/api/v1/secrets/{v1.SecretId}/versions", UriKind.Relative), new NewVersionRequest(B64(130), B64(60), B64(32), null), TestContext.Current.CancellationToken)).StatusCode);
