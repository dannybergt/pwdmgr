...
- Argon2id compiled from the reference C implementation to WASM; supports `parallelism`;
  ~50 KB WASM shipped base64-inline in the JS bundle, so no separate asset and no
  `WebAssembly.instantiateStreaming` cross-origin issue. Zero runtime dependencies.
- Proven bit-exact against seven Argon2 reference-suite vectors (`src/crypto/kdf.test.ts`),
  including m=64 MiB, in Node 24 and in headless Chromium 153.
...
client-side (`deriveKek`) and server-side (slice #5).
...
- Passphrase is NFKC-normalised before UTF-8 encoding; salt ≥ 16 random bytes.
...
- The frozen own vector (`correct horse battery staple`, salt `000102…0f`, `KDF_DEFAULT`)
  is the interoperability anchor for the .NET agent and any future library swap.
